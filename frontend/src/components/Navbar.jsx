...
const Navbar = () => {
  ...
  const navLinks = [
    { name: 'Home', path: '/' },
    { name: 'Vehicle Category', path: '/vehicles' },
  ...
  return (
    <nav className="flex items-center justify-between px-10 py-5 bg-white/80 backdrop-blur-md border-b border-gray-100 shadow-sm w-full sticky top-0 z-[100]">
      ...
      <div className="hidden lg:flex items-center gap-10">
        {navLinks.map((link) => (
          <Link 
            key={link.name}
            to={link.path} 
            className="relative text-[#07913a] font-bold text-base tracking-wide group"
          >
            {link.name}
            {/* Animated Underline */}
            <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-[#07913a] transition-all duration-300 group-hover:w-full"></span>
          </Link>
        ))}
        
        {/* Dropdown Menu */}
        <div className="relative group flex items-center gap-1.5 text-[#07913a] cursor-pointer">
          ...
          <div className="absolute top-[140%] left-0 w-56 bg-white border border-gray-100 shadow-[0_20px_50px_rgba(0,0,0,0.1)] rounded-2xl py-3 
            ...
            <div className="flex flex-col">
              <Link to="/booking/point-to-point" className="px-5 py-3 text-base text-[#07913a] hover:bg-[#07913a]/10 hover:text-[#07913a] transition-all duration-300 flex justify-between items-center group/item">
                Point-to-Point
              ...
              <Link to="/booking/package-tours" className="px-5 py-3 text-base text-[#07913a] hover:bg-[#07913a]/10 hover:text-[#07913a] transition-all duration-300 flex justify-between items-center group/item">
                Package Tours
                <span className="opacity-0 -translate-x-2 group-hover/item:opacity-100 group-hover/item:translate-x-0 transition-all duration-300">→</span>
              </Link>
              <Link to="/booking/customized-tours" className="px-5 py-3 text-base text-[#07913a] hover:bg-[#07913a]/10 hover:text-[#07913a] transition-all duration-300 flex justify-between items-center group/item">
                Customized Tours
                <span className="opacity-0 -translate-x-2 group-hover/item:opacity-100 group-hover/item:translate-x-0 transition-all duration-300">→</span>
              </Link>
            </div>
      ...
      {/* Login Button - Changed to Link if you have a login page */}
