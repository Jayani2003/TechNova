...
const CounterInput = ({ value, onChange, min = 0 }) => (
  <div className="flex items-center gap-2">
    <button type="button" onClick={() => onChange(Math.max(min, value - 1))}
      className="w-8 h-8 rounded-lg border border-slate-200 flex items-center justify-center text-slate-600 hover:bg-slate-50 transition-colors cursor-pointer font-bold text-lg">
      −
    </button>
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mb-4">Tell us how many people will be travelling.</p>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {/* Adults */}
          <div className="flex items-center justify-between bg-white border border-slate-200 rounded-xl px-4 py-3">
            <div>
              <p className="text-sm font-semibold text-slate-700">Adults</p>
          ...
          <div className="flex items-center justify-between bg-white border border-slate-200 rounded-xl px-4 py-3">
            <div>
              ...
              <p className="text-xs text-slate-400">Age 0–17</p>
            </div>
            <CounterInput
          ...
          {data.noOfChildren > 0 && (
            <div className="sm:col-span-2">
              <label className="block text-sm font-semibold text-slate-700 mb-2">Ages of Children *</label>
                ...
                {Array.from({ length: data.noOfChildren }, (_, i) => {
                  ...
                  return (
                    <div key={i} className="flex flex-col items-center gap-1">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">Child {i + 1}</span>
                      <input type="number" min="0" max="17" value={ages[i] || ""} placeholder="Age"
                        ...
                        className="w-16 text-center px-2 py-2.5 bg-white border border-slate-200 rounded-xl text-slate-800 text-sm outline-none focus:border-[#F5820D] focus:ring-2 focus:ring-[#F5820D]/20"
                      />
                    </div>
                  );
                })}
              ...
              <label className="flex items-center gap-3 cursor-pointer select-none group">
                <div className="relative">
                  ...
                  <div className={`w-6 h-6 rounded-lg border-2 flex items-center justify-center transition-all ${
                    data.babySeatNeeded ? "bg-[#F5820D] border-[#F5820D]" : "border-slate-300 bg-white group-hover:border-[#F5820D]"
                  }`}>
                    {data.babySeatNeeded && (
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400">Tick if any child needs a baby seat</p>
                </div>
              </label>
            </div>
          )}
      ...
      <div>
        ...
        <div className="space-y-3">
          {LUGGAGE_WEIGHTS.map(({ key, label, desc, icon }) => (
            <div key={key} className="flex items-center justify-between bg-white border border-slate-200 rounded-xl px-4 py-3">
          ...
          <div className="bg-white border border-slate-200 rounded-xl px-4 py-3">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-sm font-semibold text-slate-700">Custom Weight</p>
                  <p className="text-xs text-slate-400">Oversized or special luggage</p>
                </div>
              </div>
              <CounterInput
        ...
        {/* Total summary */}
        {totalLuggage > 0 && (
          <div className="mt-3 flex items-center gap-2 text-xs text-slate-500 bg-slate-50 rounded-xl px-4 py-2.5">
            <Info className="w-4 h-4 text-[#F5820D] flex-shrink-0" />
            Total: <span className="font-bold text-slate-700">{totalLuggage} piece{totalLuggage > 1 ? "s" : ""}</span> of luggage
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {VEHICLE_CATEGORIES.map((cat) => (
              <button key={cat.id} type="button" onClick={() => onChange("categoryId", cat.id)}
                ...
                <span className={`text-xs font-bold ${data.categoryId === cat.id ? "text-[#F5820D]" : "text-slate-700"}`}>
                  {cat.label}
                </span>
